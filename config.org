...
I'm having a bit of trouble making up my mind whether to use Selectrum or Ivy here.

The thing I don't like about Selectrum compared to Ido is that DEL (backspace)
doesn't go to the parent dir. To do the same thing in Selectrum, use C-M-DEL.

Ivy feels better, but it looks kind of too bare. I might just be able to fix that
with more complementary modes.
...
#+begin_src emacs-lisp
  ...
  (defun setup-selectrum ()
    ...
    ;; Save your command history on disk
...
Not very impressed with Projectile so far, but let's keep it around for now.
